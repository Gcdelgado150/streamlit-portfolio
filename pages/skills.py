...
def retrive_details(skill):
    ...
    if skills_tree[skill] == 3:
        ...
        st.write(f"If needed, I can quickly study and become proficient in using it.")

    if skills_tree[skill] == 2:
        st.write(f"{skill} is a weak 2/5!")
        st.write(f"I probably know a few things in {skill}!")
        st.write(f"I've either worked or studied a few times {skill}!")
        st.write(f"If needed I would probably have to revisit this {skill} to improve it!")

    if skills_tree[skill] == 1:
        st.write(f"{skill} is a bad 1/5!")
...
st.header(":blue[Skills]", divider="violet")
...
list_skills = sorted(list(skills_tree.keys()))
cols = st.columns([3, 1])  # 75% and 25% layout

placeholder = "All skils"
...
with cols[0]:
    ...
    option = st.selectbox(label="You can also select an specific skills to see details!",
                 options=[placeholder] + list_skills,
